...
const BottomSheet = forwardRef((props, ref) => {
  ...
  const onShow = () => {
    props.value != null && setSelectItem(props.value)
    setShow(true)
  }
  const onHide = () => {
    slideDown()
  ...
  const renderItems = ({ item, index }) => (
    <TouchableOpacity onPress={() => { props.onChooseItem(item); setSelectItem(item); onHide() }} style={{
      padding: Sizes.h16, borderBottomWidth: 1,
      borderColor: '#F5F5F5', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'
    }}>
      {props.isYear ? <Text>{item.title}</Text> : <Text>{item.title}</Text>}
            ...
            <Animated.View style={[styles.modal, { height: modalHeight, transform: [{ translateY: animation }] }]}>
              <View style={styles.title}>
                <TouchableOpacity onPress={onHide}>
                  <Image style={{}} source={images.xFilter} />
                </TouchableOpacity>
